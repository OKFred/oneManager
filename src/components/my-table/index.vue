<template>
  <view class="table_box" :style="`min-height: auto;`">
    ...
    <view class="tr bg-w my-center-vertically">
      <view v-for="({ thStyle, width, label, pinned }, index) in parentObj?.tableColumn" :key="index"
        :class="`th my-center-vertically ${pinned === 'left' ? 'pinned-left' : pinned === 'right' ? 'pinned-right' : ' '} ${pinned && index === parentObj.tableColumn.length - 1 ? 'pinned-first' : ''}`"
        :style="`${thStyle}; ${width ? 'width:' + width : ''};`">
        {{ label }}
      </view>
    </view>
    <!-- 表体 -->
    <view class="tr bg-g my-center-vertically" v-for="(obj, i) in parentObj?.tableRow" :key="i">
      <view
        v-for="({ key, label, prefix, postfix, width, pinned, tdStyle, color, backgroundColor, backgroundColor2, label2, color2 }, index) in parentObj?.tableColumn"
        :key="index"
        :class="`td my-center-vertically ${pinned === 'left' ? 'pinned-left' : pinned === 'right' ? 'pinned-right' : ''} ${pinned && i === parentObj.tableRow.length - 1 ? 'pinned-last' : pinned ? 'pinned' : ''}`"
        :style="`${tdStyle}; ${width ? 'width: ' + width : ''}; ${label2 ? ' flex-direction: column;' : ''};`"
        ...
        <view class="my-center my-center-vertically" v-if="!obj.editing">
          <text v-if="prefix && !/^http/.test(prefix) && !/^\//.test(prefix)" class="my-inline-text my-ellipsis" style="margin-right: 0.3rem;">
            {{ obj[prefix] || prefix || '' }}
          </text>
          <image v-else-if="prefix" :src="prefix" mode="widthFix"
